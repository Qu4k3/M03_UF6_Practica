...
public class Queries {
    ...
    public static String queryNewClan = "INSERT INTO `clan` (`name`, `acronym`) VALUES (?, ?);";
    /* name, acronym */

    public static String queryDropClan = "DELETE FROM `clan` WHERE `clan`.`acronym` = ?";
    /* acronym */

    public static String queryUpdatePlayerClan = "UPDATE `player` SET `id_clan` = ? WHERE `player`.`id_player` = ?";    /* id_clan, id_player */
    ...
    public static String queryListClans = "SELECT c.acronym, c.logo, c.name, c.id_clan FROM clan c ORDER BY c.acronym";
}
